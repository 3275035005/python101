  ...
  <div>
    <div style="width: 100%; height: 100vh; overflow: hidden" class="bgImg">
      <div style="width: 500px; height: 350px; background-color:rgba(9, 50, 113 ,0.2); border-radius: 10px;
        ...
        <div style="width: 260px; margin: 0 auto; padding-top: 30px">
          <div class="item" style="font-size: 20px; color: white">充电桩系统登录</div>
          <div class="item">
          ...
          <div style="padding: 10px 0; text-align: right">
            <span style="cursor: pointer; color: #049d04" @click="preReset">忘记密码</span>
            <span style="cursor: pointer; color: #049d04; margin-left: 10px" @click="goRegister">去注册</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog title="忘记密码" :visible.sync="dialogFormVisible" width="30%"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button style="background: #049d04" type="primary" @click="save">重置</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    save() {
      API.post("/manage/user/resetPassword", this.reset).then(res => {
        if (res.code === 0) {
          this.$message({
            type: "success",
            message: "重置成功"
    ...
    login() {
      ...
      API.post("/manage/user/admin_login", this.user).then(res => {
        if (res.code === 0) {
          this.$message({
            type: "success",
            message: "登录成功"
